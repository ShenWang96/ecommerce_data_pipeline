"""
experiment.py — Amazon Japan 数据源可行性验证
"""
...
RESULT = {"source": "Amazon JP", "status": "unknown", "findings": [], "products": []}
...
def main():
    print("=" * 60)
    print("Amazon JP — Feasibility Test")
    print("=" * 60)
    ...
    with sync_playwright() as p:
        ...
        url = "https://www.amazon.co.jp/s?k=bluetooth+speaker"
        ...
        for item in items[:5]:
            ...
            price = f"¥{pw.text}{pf.text}" if pw else "N/A"
            rating_el = item.select_one(".a-icon-alt")
            rating = rating_el.text.strip() if rating_el else "N/A"
            info = f"[{asin}] {title} | {price} | {rating}"
            log(info)
            RESULT["products"].append({"asin": asin, "title": title, "price_jpy": price, "rating": rating})

        screenshot = Path(__file__).parent / "amazon_jp_search.png"
