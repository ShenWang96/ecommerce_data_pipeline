"""
experiment.py — Amazon Australia 数据源可行性验证
"""
...
RESULT = {"source": "Amazon AU", "status": "unknown", "findings": [], "products": []}
...
def extract_products(soup, max_items=5):
    ...
    for item in items[:max_items]:
        ...
        price = f"AU${pw.text}{pf.text}" if pw else "N/A"
        ...
        info = f"[{asin}] {title} | {price} | {rating} | Reviews={reviews}"
        log(info)
        RESULT["products"].append({"asin": asin, "title": title, "price_aud": price, "rating": rating, "reviews": reviews})

def main():
    print("=" * 60)
    print("Amazon AU — Feasibility Test")
    print("=" * 60)
    ...
    with sync_playwright() as p:
        ...
        url = "https://www.amazon.com.au/s?k=bluetooth+speaker"
        ...
        screenshot = Path(__file__).parent / "amazon_au_search.png"
